...
class Detection:
    ...
    def estimation_pose_object(self, x_coord1, x_coord2, inc_x, inc_y, inc_x_prev, inc_y_prev):
        """
        Function to calculate the estimation pose of the object using the intersection between two
        lines.

        :param x_coord1: coordinate 'x' of the object in the previous image
        :param x_coord2: coordinate 'x' of the same object in the actual image
        :param inc_x: current displacement with the wheelchair in x
        :param inc_y: current displacement with the wheelchair in y
        :param inc_x_prev: previous state of the wheelchair in x
        :param inc_y_prev: previous state of the wheelchair in y
        :return: the intersection between the Lines in coordinates (x,y)
        """
        # Angle's detection of the object in the previous image
        angle1 = math.atan(((x_coord1-self.width/2)*math.tan(self.aperture))/(self.width/2))
        # Angle's detection of the object in the current image
        angle2 = math.atan(((x_coord2-self.width/2)*math.tan(self.aperture))/(self.width/2))

        # Construct the first Line that connect the wheelchair with the object
        # Is necessary two points
        P1 = [inc_x_prev, inc_y_prev]
        P2 = [inc_x_prev+1*math.tan(angle1), inc_y_prev+1]
        # Create the Line1
        L1 = self.line(P1, P2)

        # Construct the second Line that connect the wheelchair with the object
        # Is necessary two points
        P3 = [inc_x_prev + inc_x, inc_y_prev + inc_y]

        P4 = [inc_x_prev + inc_x + 1*math.tan(angle2), inc_y_prev + inc_y + 1]
        # Create the Line2
        L2 = self.line(P3, P4)

        # Finally, calculate the intersection between the Lines
        R = self.intersection(L1, L2)

        # If the estimation of distance of the object is negative (behind the camera),
        # we set point of intersection to None
    ...
    def extract_correspondence(self, cent1_objects, cent2_objects, Objs, pos, pos_prev, file_img1, file_img2,
                               matrix_track_prob):
                ...
                if cent1_objects[0][n] is not None and cent2_objects[0][n] is not None:
                    ...
                    while np.any(matrix_track_prob[n]):
                        ...
                        if ind_corresponding_xc1[row_max] == 0 and ind_corresponding_xc2[col_max] == 0:
                            ...
                            x_object, y_object = self.estimation_pose_object(xc1, xc2,
                                                                             pos[0], pos[1],
...
results = DET.tracking_sequence('Secuencia19')
